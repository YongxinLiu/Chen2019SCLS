...
plot_pie_familyBphylum <- function(sampleV){
    ...
    ggsave(file=paste("pie_otu_", SampAvsB, "_enriched.pdf", sep=""), p, width = 4, height = 2.5, useDingbats=F) # 1/4 of half page
    ggsave(file=paste("pie_otu_", SampAvsB, "_enriched.png", sep=""), p, width = 4, height = 2.5)
    ...
    ggsave(file=paste("pie_otu_", SampAvsB, "_depleted.pdf", sep=""), p, width = 4, height = 2.5, useDingbats=F) # 1/4 of half page
    ggsave(file=paste("pie_otu_", SampAvsB, "_depleted.png", sep=""), p, width = 4, height = 2.5)
...
if ("" == "FALSE") {
	...
	compare_data <- read.table("group_compare.txt", sep="\t", check.names=F, quote='', com='')
	colnames(compare_data) <- c("sampA", "sampB")
	# 使用第一组比较为便
	plot_pie_familyBphylum(compare_data[4,])
# 	for(i in 1:dim(compare_data)[1]){
#     plot_pie_familyBphylum(compare_data[i,])}
}	

## 基于每一组差异比较绘制饼图的代码。只需求两组共有的绘制即可。
